"""Makuuchi Rank by Era page renderer."""
...
def write_makuuchi_rank_by_era_page(
    page: Page,
    target_path: Path,
    asset_prefix: str = "",
) -> None:
...
function tracesForRows(pageConfig, rows) {
  ...
  const groupOrder = chartConfig.group_order || [...new Set(rows.map(row => row[chartConfig.group_field]))];
  return groupOrder.map(era => {
    const eraRows = rows.filter(row => row[chartConfig.group_field] === era);
    return {
      type: "bar",
      name: era,
      x: eraRows.map(row => row[chartConfig.x_field]),
      y: eraRows.map(row => numeric(row[chartConfig.y_field])),
      customdata: eraRows.map(row => numeric(row.total)),
      hovertemplate:
        "Era=%{fullData.name}<br>" +
        "Rank=%{x}<br>" +
        "Appearances=%{y}<br>" +
        "Rank total=%{customdata}<extra></extra>"
    };
  });
}
...
function layoutFor(pageConfig, rows) {
  const chartConfig = pageConfig.chart;
  const xValues = [...new Set(rows.map(row => row[chartConfig.x_field]))];
  return {
    title: null,
