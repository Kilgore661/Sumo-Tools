...
| Equelo package API and cleanup | Open | Tidy the `analysis/equelo` package docs and code layout so it has a clear public API and only the code needed to implement or support that API. Update consumers that should use the API. In particular, review `src.analysis.sumo_history.basho_results.kyujo_hack` and every current or planned use of it, including Basho Results and Career Comparisons rating data production. |
