...
| Published Artifact type and renderer menagerie | Open | Audit the current set of PA kinds, artifact dataclasses, renderer strings and custom runtime renderers. `3.3 Career Comparisons` adds another custom chart PA, making it more important to distinguish genuine PA-specific analytical structure from accidental one-off plumbing. Decide whether `04.4 Published Artifact Model.md` needs a concise registry/table of approved PA terminal forms and renderer kinds, or whether this should remain an implementation lookup until more pressure accumulates. |
