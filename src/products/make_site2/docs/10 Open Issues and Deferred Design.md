...
| Sort out 7.1 Basho Results model and table structure | Open | Current agreed model is in `Basho Results Table (7.1) Model.md`, with supporting notes in `Basho Results (7.1) Working Note.md`, `Table Structure Design Gap.md` and `Tables as Dicts.md`. 7.1 should remain Basho Results and own represented-basho result state, including in-progress/current versus completed/after wording. The table-like PA model has not yet fully captured hierarchical column structure; do not treat current `ColumnGroup` code as the final design. |
