...
| Stress-test 7.1 Basho Results redesign | Open | The settled 7.1 model has been folded into `04.5 Basho Results Model.md`, and BZ Error / DeltaBZ plus RBBP are now specified in `02 Specification.md` and designed in `04.5`. The remaining work is implementation and stress-testing: emit or derive BZ Error direction/magnitude and RBBP values, render and sort the new terminal paths, check in-progress and no-successor-banzuke cases, and decide how much transitional compact-result parsing may remain. |
