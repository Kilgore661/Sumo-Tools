...
| 6.3.1 selected BP versus public-category aggregation | Deferred / TBD | Current Win Probability by Standing output is internally consistent: Observed and Equelo both use the same curated selected sideless BP domain and display-select canonical sanyaku BPs such as `Y1/O1/S1/K1`. Decide whether the public page should instead aggregate ordinary numbered sanyaku BPs into title/public-category bins such as `O`/`Ozeki`, while keeping historical/low-support BP domain exclusions such as `M18-M22` and `J13-J24` separate. |
