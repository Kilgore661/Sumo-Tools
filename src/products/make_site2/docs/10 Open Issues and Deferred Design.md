...
| Sort out 7.1 Basho Results model and table structure | Open | Focused working notes are in `7.1 Basho Results Working Note.md` and `Table Structure Design Gap.md`. 7.1 should remain Basho Results and own represented-basho result state, including in-progress/current versus completed/after wording. The table-like PA model has not yet fully captured hierarchical column structure; do not treat current `ColumnGroup` code as the final design. |
