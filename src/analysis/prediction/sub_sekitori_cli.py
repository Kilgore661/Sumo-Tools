"""Run the sub-sekitori-only evaluation of Proposal 1 Basic Elo."""
...
from .sekitori import evaluate_sub_sekitori_bouts
from .sub_sekitori_output import write_sub_sekitori_outputs
...
def main() -> None:
    ...
    output_directory = (
        ...
        / f"sub_sekitori_only_{str(result.definition.start_date).replace('/', '_')}_to_{str(end_date).replace('/', '_')}"
    )
    write_sub_sekitori_outputs(
        result,
        baseline,
        output_directory,
        HistorySource(path=str(history_zip), sha256=_sha256(history_zip)),
    )
    print(f"Wrote sub-sekitori artifacts to {output_directory.resolve()}")
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("files/output/prediction/sub_sekitori_only"),
    )
